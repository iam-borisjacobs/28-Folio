...
<div class="py-6" x-data="{ tab: 'content' }">
    <div class="w-full px-6">
        ...
        <div class="flex flex-col mb-8">
            <h1 class="text-5xl font-bold text-white mb-4">Edit Project</h1>
            ...
            <div class="bg-[#1e293b] rounded-lg shadow-xl overflow-hidden border border-gray-700">
                ...
                <div class="p-8">
                     ...
                     <div x-show="tab === 'content'" class="space-y-8" x-data="{ currentLang: '{{ $languages->first()->code ?? 'en' }}' }">
                        ...
                        @foreach($languages as $language)
                            @php
                                $translation = $project->translation($language->code);
                            @endphp
                            <div x-show="currentLang === '{{ $language->code }}'" style="display: none;">
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea id="short_description_{{ $language->code }}" name="translations[{{ $language->code }}][description]" rows="3"
                                        class="block w-full rounded-md border-gray-700 bg-gray-900 text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-lg px-5 py-4"
                                        placeholder="Write a short description...">{{ old('translations.'.$language->code.'.description', $translation?->description) }}</textarea>
                                ...
                                <div>
                                    ...
                                    <div class="rounded-md border border-gray-700 bg-gray-900 overflow-hidden">
                                        ...
                                        <textarea id="full_description_{{ $language->code }}" name="translations[{{ $language->code }}][content]" rows="10"
                                            ...
                                            placeholder="Write the full description...">{{ old('translations.'.$language->code.'.content', $translation?->content) }}</textarea>
                                    </div>
                    ...
                    <div x-show="tab === 'media'" class="space-y-8" style="display: none;">
                        <div class="bg-gray-900 rounded-lg p-8 border border-gray-700 border-dashed text-center">
                            ...
                            @if($project->featured_image)
                                <div class="mb-6 flex justify-center">
                                    <img src="{{ Storage::url($project->featured_image) }}" alt="Current Featured Image" class="h-48 w-auto rounded border border-gray-600">
                                </div>
                            @endif
                        ...
                        <div class="bg-gray-900 rounded-lg p-8 border border-gray-700 border-dashed text-center">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                    @foreach($project->images as $image)
                                        <div class="relative group">
                                            <img src="{{ Storage::url($image->image_path) }}" class="w-full h-32 object-cover rounded border border-gray-600">
                            ...
                            <input id="gallery_images" type="file" name="gallery_images[]" multiple accept="image/*" class="block w-full text-base text-gray-400 file:mr-4 file:py-3 file:px-6 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700">
                             <p class="mt-3 text-sm text-gray-500">Upload multiple images. New images will be appended to the gallery.</p>
                        </div>
                    </div>
                    ...
                    <div x-show="tab === 'organization'" class="space-y-8" style="display: none;">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                 @forelse($categories as $category)
                                    <label class="inline-flex items-center p-4 rounded bg-gray-900 border border-gray-700 cursor-pointer hover:border-indigo-500 transition-colors">
                                        <input type="checkbox" name="categories[]" value="{{ $category->id }}" class="rounded border-gray-700 bg-gray-800 text-indigo-600 shadow-sm focus:ring-indigo-500 h-5 w-5" {{ in_array($category->id, old('categories', $project->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                 @forelse($tags as $tag)
                                    <label class="inline-flex items-center p-4 rounded bg-gray-900 border border-gray-700 cursor-pointer hover:border-indigo-500 transition-colors">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="rounded border-gray-700 bg-gray-800 text-indigo-600 shadow-sm focus:ring-indigo-500 h-5 w-5" {{ in_array($tag->id, old('tags', $project->tags->pluck('id')->toArray())) ? 'checked' : '' }}>
                       ...
                       <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
                           ...
                           <div class="flex items-center pb-4">
                               <label class="inline-flex items-center cursor-pointer">
                                   <input type="checkbox" name="is_featured" value="1" class="rounded border-gray-700 bg-gray-900 text-indigo-600 shadow-sm focus:ring-indigo-500 h-6 w-6" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }}>
