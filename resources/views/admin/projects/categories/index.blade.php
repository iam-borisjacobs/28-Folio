...
<div class="py-6">
    <div class="w-full px-6">
        ...
        <div class="flex flex-col mb-8">
            <h1 class="text-5xl font-bold text-white mb-4">Project Categories</h1>
            ...
            <div class="lg:col-span-1">
                <div class="bg-[#1e293b] rounded-lg shadow-xl overflow-hidden border border-gray-700 sticky top-6">
                    <div class="p-8 border-b border-gray-700">
                        <h3 class="text-2xl font-bold text-white">Create New Category</h3>
                    </div>
                    <form action="{{ route('admin.projects.categories.store') }}" method="POST" class="p-8 space-y-6">
                        @csrf
                        <div>
                            ...
                            <tbody class="divide-y divide-gray-700">
                                @forelse($categories as $category)
                                    <tr class="hover:bg-gray-800/50 transition-colors">
                                        <td class="px-8 py-6 text-white text-lg font-medium">{{ $category->name }}</td>
                                        <td class="px-8 py-6 text-gray-400 text-lg">{{ $category->slug }}</td>
                                        <td class="px-8 py-6 text-right">
                                            <div class="flex items-center justify-end space-x-6">
                                                <a href="{{ route('admin.projects.categories.edit', $category) }}" class="text-indigo-400 hover:text-indigo-300 text-lg font-medium hover:underline">Edit</a>
                                                
                                                <form action="{{ route('admin.projects.categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Delete this category?');">
                                                    @csrf
                                                    @method('DELETE')
